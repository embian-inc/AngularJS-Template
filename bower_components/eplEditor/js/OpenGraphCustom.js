...
//                var delta = dx * (type != "diamond" && type != "oval");
    ...
    setFillAndStroke = function (o, params) {
        var node = o.node,
            attrs = o.attrs,
            vis = node.style ? node.style.visibility : '';
 ...
 * @author <a href="mailto:hrkenshin@gmail.com">Seungbaek Lee</a>
 */
OG.shape.VerticalGroupShape = function (label) {
	OG.shape.VerticalGroupShape.superclass.call(this, label);
    this.CONNECTABLE = true;
    this.TaskType = "EventBase";
	this.SHAPE_ID = 'OG.shape.VerticalGroupShape';
...
OG.shape.VerticalGroupShape.prototype = new OG.shape.GroupShape();
OG.shape.VerticalGroupShape.superclass = OG.shape.GroupShape;
OG.shape.VerticalGroupShape.prototype.constructor = OG.shape.VerticalGroupShape;
OG.VerticalGroupShape = OG.shape.VerticalGroupShape;
 ...
 * @override
 */
OG.shape.VerticalGroupShape.prototype.createShape = function () {
	if (this.geom) {
		return this.geom;
	}
...
OG.shape.bpmn.A_OperateTask = function (label) {
    OG.shape.bpmn.A_ServiceTask.superclass.call(this);

    this.SHAPE_ID = 'OG.shape.bpmn.A_OperateTask';
    this.label = label;
    this.CONNECTABLE = false;
    this.GROUP_COLLAPSIBLE = false;
    this.LoopType = "None";
...
OG.shape.bpmn.A_OperateTask.prototype = new OG.shape.bpmn.A_Task();
OG.shape.bpmn.A_OperateTask.superclass = OG.shape.bpmn.A_Task;
OG.shape.bpmn.A_OperateTask.prototype.constructor = OG.shape.bpmn.A_OperateTask;
OG.A_OperateTask = OG.shape.bpmn.A_OperateTask;
 ...
 * @override
 */
OG.shape.bpmn.A_OperateTask.prototype.createShape = function () {
    if (this.geom) {
        return this.geom;
    }
    ...
    */
	...
	getElementsByBBox: function (envelope) {
		...
		$(this.getRootElement()).find("[_type=" + OG.Constants.NODE_TYPE.SHAPE + "]").each(function (index, element) {
			if (element.shape && element.shape.geom && envelope.isContainsAll(element.shape.geom.getVertices())) {
	...
	getElementMapByBBox: function (envelope, _elements, groupId) {
		...
		$(this.getRootElement()).find("[_type=" + OG.Constants.NODE_TYPE.SHAPE + "]").each(function (index, element) {
            if( element && element.shape && element.shape.geom ) {
...
OG.renderer.RaphaelRenderer.prototype._drawLabel = function (position, text, size, style, id, isEdge) {
	...
	if (_style["label-direction"] === 'vertical') {
		...
		switch (_style["vertical-align"]) {
		case "top":
			y = geom.getBoundary().getUpperCenter().y  + 5;
			break;
		case "bottom":
...
OG.renderer.RaphaelRenderer.prototype.drawShape = function (position, shape, size, style, id) {
    ...
    // VerticalGroupShape 에 도형을 그리기 위하여 어쩔수 없이 추가함 : hyungkook
    if(shape instanceof OG.shape.VerticalGroupShape){
        if(groupNode.shape.TaskType != 'None')
            this.drawTaskType(groupNode);
    }
	...
	if(groupNode.shape.shapeTitle){
        if(groupNode.geom.titleText) {
            var ur_x = groupNode.geom.boundary._upperLeft.x;
            var ur_y = groupNode.geom.boundary._upperLeft.y;

            var style = groupNode.geom.titleStyle;
            var titleText = groupNode.geom.titleText;
            var positionX = groupNode.geom.titleX;
            var positionY = groupNode.geom.titleY;
            var shapeText = this.drawText([ur_x + positionX, ur_y + positionY], titleText, [130, 20], style, id + '_shapeTitle');

            groupNode.appendChild(shapeText);
        }
    }

	if(shape instanceof OG.shape.HorizontalLaneShape
		|| shape instanceof OG.shape.VerticalLaneShape
        || shape instanceof OG.shape.VerticalGroupShape){
	...
	if (_tempStyle['label-direction'] && _tempStyle['vertical-align'] === 'top') {
		...
		if (_tempStyle['label-direction'] === 'vertical') {
			...
			if(_tempStyle['title-size']){
				...
				titleLine = new OG.geometry.Line(
					[boundary.getUpperLeft().x, boundary.getUpperLeft().y + 35],
					[boundary.getUpperRight().x, boundary.getUpperRight().y + 35]
				);
				group_hidden = new OG.geometry.Rectangle(boundary.getUpperLeft(), (boundary.getUpperRight().x - boundary.getUpperLeft().x), 35);
			}
		}
...
OG.renderer.RaphaelRenderer.prototype.redrawShape = function (element, excludeEdgeId) {
	...
	if (element && element.shape.geom) {
        ...
        // VerticalGroupShape 에 도형을 그리기 위하여 어쩔수 없이 추가함 : hyungkook
        if(element.shape instanceof OG.shape.VerticalGroupShape){
            if(element.shape.TaskType != 'None')
                this.drawTaskType(element);
        }
        ...
        if(element.shape.shapeTitle) {
            if(element.shape.geom.titleText) {
                var ur_x = element.shape.geom.boundary._upperLeft.x;
                var ur_y = element.shape.geom.boundary._upperLeft.y;

                var style = element.shape.geom.titleStyle;
                var titleText = element.shape.geom.titleText;
                var positionX = element.shape.geom.titleX;
                var positionY = element.shape.geom.titleY;
                var shapeText = this.drawText([ur_x + positionX, ur_y + positionY], titleText, [100, 20], style, element.id + '_shapeTitle');
...
OG.renderer.RaphaelRenderer.prototype.removeTerminal = function (element) {
    ...
    //$(this.getRootElement()).find("[_type=CUSTOM_CONTROL]").each(function(n, selectedItem){
    //    selectedItem.remove();
    //});
};
...
OG.renderer.RaphaelRenderer.prototype.drawTaskType = function (element) {
	...
	var me = this, rElement = this._getREleById(OG.Util.isElement(element) ? element.id : element),
		geometry = rElement ? rElement.node.shape.geom : null,
		envelope, _upperLeft, _bBoxRect, _rect, _rect1, drawBackground,
		_size = me._CONFIG.COLLAPSE_SIZE,
		_hSize = _size / 2;
    ...
    case "Device":
        _rect1 = this._PAPER.image("images/activity_status/devicedata.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Event":
        _rect1 = this._PAPER.image("images/activity_status/eventdata.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Schedule":
        _rect1 = this._PAPER.image("images/activity_status/windowcondition.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "EventBase":
        _rect1 = this._PAPER.image("images/activity_status/eventdefine.svg", _upperLeft.x + 5, _upperLeft.y + 5, 30, 30);
        break;
    case "EventOccur":
        _rect1 = this._PAPER.image("images/activity_status/triggeredcondition.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "EventTime":
        _rect1 = this._PAPER.image("images/activity_status/windowcondition.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        ...
        break;
    case "EventGroup":
        _rect1 = this._PAPER.image("images/activity_status/group.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "EventMap":
        _rect1 = this._PAPER.image("images/activity_status/locationevent.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "EventEPL":
        _rect1 = this._PAPER.image("images/activity_status/EPL.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Email":
        _rect1 = this._PAPER.image("images/activity_status/email.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Push":
        _rect1 = this._PAPER.image("images/activity_status/push.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Sms":
        _rect1 = this._PAPER.image("images/activity_status/SMS.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    case "Control":
        _rect1 = this._PAPER.image("images/activity_status/control.svg", _upperLeft.x + 5, _upperLeft.y + 5, 20, 20);
        drawBackground = true;
        break;
    }
    ...
    if(drawBackground){
        _rect = this._PAPER.rect(envelope.getUpperLeft().x, envelope.getUpperLeft().y, 30 , 30);
        _rect.attr("fill", "#4c4c4c");
        _rect.attr("fill-opacity", "1");
        _rect.attr("stroke", "#4c4c4c");
        this._add(_rect, rElement.id + OG.Constants.TASKTYPE_SUFFIX + '_RECT');
        _rect1.insertAfter(_rect);
        _rect.insertBefore(_rect1);
...
OG.renderer.RaphaelRenderer.prototype.drawExceptionType = function (element) {
	...
	var me = this, rElement = this._getREleById(OG.Util.isElement(element) ? element.id : element),
		geometry = rElement ? rElement.node.shape.geom : null,
		envelope, _upperLeft, _bBoxRect, _rect, drawBackground,
		_size = me._CONFIG.COLLAPSE_SIZE,
		_hSize = _size / 2;
	...
	envelope = geometry.getBoundary();
	//_upperRight = envelope.getUpperRight();
    var _lowerRight = envelope.getLowerRight();

    switch(element.shape.exceptionType){
        case "error":
            _rect = this._PAPER.image("images/activity_status/error.svg", _lowerRight.x - 30, _lowerRight.y  - 30, 25, 25);
            drawBackground = 'error';
            break;
        case "complete":
            drawBackground = 'complete';
            break;
    }
    if(drawBackground == 'error' ){
        var taskBackground = this._getREleById(rElement.id + OG.Constants.TASKTYPE_SUFFIX + '_RECT');
        taskBackground.attr("fill", "#e34345");
        taskBackground.attr("stroke", "#e34345");
    }else if(drawBackground == 'complete' ){
        var taskBackground = this._getREleById(rElement.id + OG.Constants.TASKTYPE_SUFFIX + '_RECT');
        taskBackground.attr("fill", "#47c1c1");
        taskBackground.attr("stroke", "#47c1c1");
    }
    if( _rect ){
...
OG.handler.EventHandler.prototype = {
			...
			dblclick: function (event) {
				...
				if (element.shape.isCollapsed === false) {
					...
					if ($(element).attr("_shape") === OG.Constants.SHAPE_TYPE.HTML) {
						...
						$(eventObject).parents('[id$=_FROMLABEL]').each(function (idx, item) {
							$(item).find("text").each(function (idx2, item2) {
								bBox = me._RENDERER.getBBox(item2);
								left = bBox.x - 10;
								top = bBox.y;
						...
						// 끝점 라벨인 경우
						$(eventObject).parents('[id$=_TOLABEL]').each(function (idx, item) {
							$(item).find("text").each(function (idx2, item2) {
								bBox = me._RENDERER.getBBox(item2);
								left = bBox.x - 10;
								top = bBox.y;
	...
	checkAutoAttach: function(element, bBoxArray, dx, dy, start, offset, event){
			...
			&& element.shape instanceof OG.shape.bpmn.Event)
		{
            ...
            var eventObject = (event.target)?event.target:event.srcElement;
			targetElement = $(eventObject).parent("g")[0];
				...
				if("start" == $(this).data("rubber_band_status")){
					...
					if (first) {
						...
						$(me._RENDERER.getRootElement()).find("[_type=" + OG.Constants.NODE_TYPE.SHAPE + "]").each(function (index, element) {
							if (element && element.shape && element.shape.geom && envelope.isContainsAll(element.shape.geom.getVertices())) {
								elements.push(element);
							}
						});
	...
	setEnableHotKey: function (isEnableHotKey) {
		...
		if (isEnableHotKey === true) {
			...
			$(me._RENDERER.getContainer()).bind("keydown", function (event) {
				...
				if (!/^textarea$/i.test(eventObject.tagName) && !/^input$/i.test(eventObject.tagName)) {
					// Delete : 삭제
...
OG.graph.Canvas = function (container, containerSize, backgroundColor, backgroundImage) {
	...
	this._CONFIG = {
		...
		DEFAULT_STYLE: {
			...
			GROUP         : { stroke: "black", fill: "none", "fill-opacity": 0, "label-position": "bottom", "text-anchor": "middle", "vertical-align": "middle" },
		...
		childShape = function (node, isRoot) {
			$(node).children("[_type=SHAPE]").each(function (idx, item) {
				...
				if(item.shape instanceof OG.shape.HorizontalLaneShape
                    ...
                    || item.shape instanceof OG.shape.VerticalGroupShape){
