...
(function(exports, global) {
    ...
    angular.module("kt.ui.accordion", []).constant("accordionConfig", {
        ...
        return {
            ...
            transclude: true,
            replace: true,
            template: '<div class="panel panel-default">       <div class="panel-heading" ><a class="accordion-toggle" ng-click="isOpen = !isOpen" accordion-transclude="heading">{{heading}}</a></div>       <div class="panel-collapse" collapse="!isOpen">         <div class="panel-body" ng-transclude></div>  </div>     </div>',
            scope: {
                heading: "@",
                isOpen: "=?",
                isDisabled: "=?"
            ...
            compile: function compile(tElement, tAttrs, transclude) {
                return function($scope, iElm, iAttrs, controller) {
                    ...
                    function init() {
                        ...
                        $scope.$watch("data", function(newScopeData, oldScopeData) {
                            var msg = $translate.instant("GRID.NO-DATA") == "GRID.NO-DATA" ? "조회된 데이터가 없습니다." : $translate.instant("GRID.NO-DATA");
    ...
    angular.module("kt.ui.modal", [ "kt.ui.dialog" ]).directive("ktModal", [ "$parse", "$modal", "$timeout", function($parse, $modal, $timeout) {
        ...
        return {
            ...
            template: '<div class="modal-backdrop fade" ng-class="{in: animate}" ng-style="{\'z-index\': 500 + index*10}" ng-click="close($event)"></div>',
            link: function(scope, element, attrs) {
    ...
    angular.module("kt.ui.tooltip", []).factory("$position", [ "$document", "$window", function($document, $window) {
        ...
        return {
            ...
            template: '<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }"> 	  <div class="tooltip-arrow"></div> 	  <div class="tooltip-inner" ng-bind="content"></div> 	</div>'
        };
        ...
        return {
            ...
            template: '<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }"> 	  <div class="tooltip-arrow"></div> 	  <div class="tooltip-inner" ng-bind-html-unsafe="content"></div> 	</div>'
        };
    ...
    angular.module("kt.ui.tree", []).controller("treeCtrl", [ "$scope", "$element", function($scope, $element) {
        ...
        return {
            ...
            controller: "treeCtrl",
            template: '<script type="text/ng-template"  id="tree_item_renderer.html"> 				<ins ng-click="toggle(treeData)" class="kt-tree-icon kt-tree-ocl">&nbsp;</ins> 				<a href="" ng-click="click(treeData, $event, selectable)" ng-mouseover="hover($event)" ng-class="checkSelect(treeData)" ng-init="setId(treeData)" id="{{treeData.$$id}}"> 					<ins class="kt-tree-icon kt-tree-themeicon">&nbsp;</ins>{{treeData.text}} 				</a> 				<ul> 					<li ng-repeat="treeData in treeData.children" ng-class="addClass(treeData, $last)" ng-include="\'tree_item_renderer.html\'"></li> 				</ul> 				</script> 				<div class="kt-tree kt-tree-default"> 				<ul class="unstyled">					<li ng-repeat="treeData in data" ng-class="addClass(treeData, $last)" ng-include="\'tree_item_renderer.html\'"></li>				</ul></div>',
